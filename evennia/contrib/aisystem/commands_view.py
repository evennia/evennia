...
from evennia.contrib.aisystem.utils import (is_aiplayer, is_browsing,
    is_browsing_blackboard, is_agent_set_up, tree_from_name, node_from_name,
    agent_from_name, display_node_in_tree, display_node_in_bb, 
    display_bb_globals)
    ...
    def func(self):
        ...
        if names and names[0] == "globals":
            ...
            display_bb_globals(self.caller,
                self.caller.player.aiwizard.agent.db.ai)
            return True
        ...
        if names and names[0] == "bb":
            ...
            if not is_browsing_blackboard(self.caller.player, msg):
                return False
            bb = self.caller.player.aiwizard.agent.db.ai
...
class CmdStatus(Command):
    ...
    def func(self):
        ...
        for watched in wiz.watching:
            watched_tree = ScriptDB.get_id(watched.hash[4:])
            s += s_indent + "'{0}'(\"{1}\") ".format(watched.hash[0:3],
                watched.name) + "in tree {0} (id {1})".format(
                watched_tree.name, watched_tree.id)
...
class CmdGo(Command):
    ...
    def func(self):
        ...
        if names[0] == "bb":
            ...
            if not is_browsing_blackboard(self.caller.player, " go to the " +
                "origin tree of the currently browsed blackboard. To " +
                "go to a blackboard, use the @aibb command"):
                return False
            ...
            if isinstance(agent, ObjectDB):
                ...
                raise TypeError ("Unknown type for AI agent {0} ".format(
                    agent.name) + "(id {0}).".format(agent.id))
...
class CmdBB(Command):
    ...
    def func(self):
        ...
        if names[0] == 'object':
            ...
            self.caller.msg("The first argument you have provided for the " +
        ...
        if self.caller.player.aiwizard.agent == agent:
            self.caller.msg("Already browsing the blackboard of {0} ".format(
                obj_type_name) + "\"{0}\" (id {1}).".format(agent.name,
                agent.id))
            return True
    ...
    def func(self):
        ...
        if isinstance(node, CompositeNode):
            if len(node.children) != 1 and not arg:
                ...
                if all([x in string.digits for x in arg[1:]]) and (
                ...
                # check if the name is a hash
                    ...
                    if len(children) == 1:
                        ...
                        s = ("Multiple children of node '{0}'(\"{1}\")".format(
                            node.hash[0:3], node.name) + " share the name " +
                            "of {0}. Please choose from among ".format(name) +
                            "these hashes:\n")
                    ...
                    self.caller.msg("Node '{0}'(\"{1}\")".format(
                        ...
                        "with a hash or name of {0}. Cannot ".format(name) +
                        "perform descent.")
                    return False
        else:
            child = node.children
