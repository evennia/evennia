...
class TypedObject(SharedMemoryModel):
    ...
    def return_examine(self, viewer, cmdsets, *args, **kwargs):
        """
        This method is used as the interface to the @examine command. It calls methods following the pattern:
            return_examine_<name> using the names stored in examine_hooks.
        Args:
            viewer: The Account or Character examining this object.
            cmdsets: The cmdsets generated by @examine. Used only for the cmdsets examine.
            *args:
            **kwargs:
        Returns:
            results (str): The text to display.
        """
        message = list()
        message.append('-' * 78)
        for hook in self.examine_hooks:
            results = getattr(self, 'return_examine_%s' % hook)(viewer, cmdsets, *args, **kwargs)
            if results:
                message.append(results)
        message.append('-' * 78)
        return "\n".join(str(l) for l in message)

    def return_examine_locks(self, viewer, cmdsets, *args, **kwargs):
        """
        Generates basic information about locks for return_examine.
        Args:
            viewer: The Account or Character examining this object.
            cmdsets: The cmdsets generated by @examine. Used only for the cmdsets examine.
            *args:
            **kwargs:
        Returns:
            results (str): Text describing this object's Locks.
        """
    ...
    def return_examine_scripts(self, viewer, cmdsets, *args, **kwargs):
        scripts = self.scripts.all()
        return "|wScripts|n:\n %s" % scripts if scripts else None

    def return_examine_tags(self, viewer, cmdsets, *args, **kwargs):
        tags_string = fill(", ".join("%s[%s]" % (tag, category)
                                     for tag, category in self.tags.all(return_key_and_category=True)), indent=5)
        return "|wTags[category]|n: %s" % tags_string.strip() if tags_string else None

    def return_examine_attributes(self, viewer, cmdsets, *args, **kwargs):
        attrs = self.attributes.all()
        if not attrs:
            return
        return "|wPersistent Attributes[category]|n:\n" + "\n".join(a.pretty_format() for a in attrs)

    def return_examine_nattributes(self, viewer, cmdsets, *args, **kwargs):
        nattrs = self.nattributes.all(return_tuples=True)
        if not nattrs:
            return
