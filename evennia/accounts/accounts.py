...
class DefaultAccount(with_metaclass(TypeclassBase, AccountDB)):
    ...
    * Hook methods
    ...
    # Used to determine the hooks that will be called by @examine, in order.
    # return_examine() below uses this.
    examine_hooks = ('base', 'locks', 'commands', 'scripts', 'tags', 'attributes', 'nattributes')

    def return_examine_base(self, cmdsets, viewer, *args, **kwags):
        """
        Generates basic information about Accounts for @examine to see. Called by return_examine.
        Args:
        ...
        message.append("|wEmail|n: |c%s|n" % self.email)
        message.append("|wTypeclass|n: %s (%s)" % (self.typename, self.typeclass_path))
        perms = self.permissions.all()
        perms_string = (", ".join(perms)) if perms else '<None>'
        if self.is_superuser:
            perms_string += " [Superuser]"
        message.append("|wPermissions|n: %s" % perms_string)
    ...
    def return_examine_commands(self, viewer, cmdsets, *args, **kwargs):
        ...
        if not (len(self.cmdset.all()) == 1 and self.cmdset.current.key == "_EMPTY_CMDSET"):
            ...
            cmdsetstr = fill(", ".join(avail_cmdset), indent=2)
